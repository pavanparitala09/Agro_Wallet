...
const { deleteFileFromCloudinary } = require("../utils/cloudinaryService");
...
function buildBillFilters(query, userId) {
  ...
  if (query.search) {
    ...
    filters.$or = [{ title: regex }, { vendorName: regex }, { notes: regex }];
  }
...
async function createBill(req, res) {
  try {
    ...
    if (!sectionId || !amount || !billDate || !status) {
      return res
        .status(400)
        .json({ message: "Section, amount, billDate and status are required" });
    }
    ...
    let imageUrl;
    if (req.file) {
      // multer-storage-cloudinary returns path with the Cloudinary URL
    ...
    const bill = await Bill.create({
      ...
      sectionId,
      vendorName,
      title: title && title.trim() ? title : "Untitled Bill",
      amount,
      billDate,
      status,
      paidAmount: paidNum,
    ...
    console.log("Bill created with imageUrl:", bill.imageUrl);
...
async function updateBill(req, res) {
  try {
    ...
    // Handle image: prioritize new upload over existing URL
    if (req.file) {
      updates.imageUrl = req.file.path;
      console.log("New image uploaded in update:", updates.imageUrl);

      // Get the old bill to delete previous image if a new one is uploaded
      const oldBill = await Bill.findOne({ _id: id, userId: req.userId });
      if (oldBill && oldBill.imageUrl) {
        console.log("Deleting old image:", oldBill.imageUrl);
        // Delete old image from Cloudinary
        await deleteFileFromCloudinary(oldBill.imageUrl);
      }
    } else if (updates.existingImageUrl) {
      // If no new file but existing image URL provided, use it
      console.log("Preserving existing image:", updates.existingImageUrl);
      updates.imageUrl = updates.existingImageUrl;
      delete updates.existingImageUrl; // Remove the temporary field
    } else {
      console.log("No image change - preserving existing");
    }
    // If neither req.file nor existingImageUrl, the existing imageUrl is preserved (not touched in update)
    ...
    if (
      updates.status === "partial_paid" &&
      typeof updates.paidAmount !== "undefined"
    ) {
    ...
    console.log("Bill updated, imageUrl:", bill.imageUrl);
    res.json(bill);
  } catch (err) {
...
async function deleteBill(req, res) {
  try {
    ...
    if (bill.imageUrl) {
      await deleteFileFromCloudinary(bill.imageUrl);
    }
